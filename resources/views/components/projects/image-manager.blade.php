    ...
    <div x-data class="flex gap-3 mt-2 flex-wrap">
        @foreach($project->images as $img)
            <div class="relative group w-24 h-24 rounded-lg overflow-hidden border {{ $img->is_primary ? 'ring-2 ring-accent' : 'border-gray-200' }}">
                <img src="{{ asset($img->image_path) }}" class="w-full h-full object-cover" alt="{{ $img->caption }}">

                @if($img->is_primary)
                    <span class="absolute bottom-0 inset-x-0 bg-accent text-white text-[10px] text-center py-0.5 font-semibold">Cover</span>
                @else
                    <form method="POST" action="{{ route('projects.images.primary', $img) }}" class="absolute bottom-0 inset-x-0">
                        @csrf
                        @method('PATCH')
                ...
                <button
                    type="button"
                    @click="$dispatch('open-confirm', { message: 'Delete this photo?', action: '{{ route('projects.images.destroy', $img) }}', method: 'DELETE' })"
