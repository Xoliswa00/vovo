...
<div {{ $attributes->merge(['class' => 'card-brand h-full']) }} data-aos="zoom-in">
    ...
    <div class="flex flex-col flex-1 p-6">
        ...
        @endif
        <h5 class="font-heading font-bold text-navy mb-1">{{ $project->title }}</h5>
        <p class="text-sm text-muted flex-1 mb-4">
            {{ \Illuminate\Support\Str::limit($project->summary ?: $project->description, 96) }}
        </p>
        ...
        <div class="flex items-center justify-between gap-3 mt-auto">
            ...
            <a href="{{ route('projects.show.public', $project) }}" class="btn-brand-outline btn-brand-sm whitespace-nowrap">
                View project
            </a>
        </div>
    </div>
